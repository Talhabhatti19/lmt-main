...
const StepSeven = () => {
  ...
  const handleSubmit = () => {
    if (phone) {
      localStorage.setItem('freelancerPhone_number', phone);
      navigate("/signup/freelancer/date")
    }
  };
  return (
    ...
    <div className='register-container' style={{boxShadow:"none", background:"transparent"}}>
      <Link to="/signup/frelance/detail" style={{display:"flex", alignItems:"center", gap:"10px", fontWeight:"bold", color:"black"}}>
      ...
      <div className="innerphone" style={{border:"1px solid #DEE3FC", borderRadius:"100px"}}>
        ...
        <input type="number" value={phone} onChange={(event)=>{setPhone(event.target.value)}} style={{border:"none", outline:"none"}} required/>
      </div>
